...
void modificarNome(const char* arquivo, const char* novoNome) {
    ...
    long int tamanho = ftell(fpOriginal); // Obtém o tamanho do arquivo
    fseek(fpOriginal, 0, SEEK_SET); // Move o cursor para o início do arquivo
    ...
    while (ftell(fpOriginal) < tamanho) {
        ...
        if (strstr(linha, "USERNAME: ") != NULL && strstr(linha, usuario) != NULL) {
            usuarioEncontrado = 1;
            fputs(linha, fpTemporario); // Copia a linha USERNAME para o arquivo temporário

            // Atualiza o nome do usuário
            for (int i = 0; i < 12; i++) {
                fgets(linha, sizeof(linha), fpOriginal); // Lê as próximas 12 linhas
                fputs(linha, fpTemporario);
            }
